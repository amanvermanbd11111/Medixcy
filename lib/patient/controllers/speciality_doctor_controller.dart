...
class SpecialityDoctorController extends GetxController {
  ...
  loadMoreFunc(city) async {
    if (nextUrl.value != "null") {
      ...
      update();
      final response =
          await get(Uri.parse("${nextUrl.value}&department_id=$id&lat=$lat&lon=$long&city_id=${city ?? 0}"));
      print(response.request!.url);
  ...
  void onInit() {
    ...
    scrollController.addListener(() {
      if (scrollController.position.pixels == scrollController.position.maxScrollExtent &&
        ...
        String selected_city1 ="${StorageService.readData(
            key: LocalStorageKeys
                .sortCityId) ??
            "0"}";

        print(selected_city1);
        loadMoreFunc(selected_city1);
      }
    });
  }
