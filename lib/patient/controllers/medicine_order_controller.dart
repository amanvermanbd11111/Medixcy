...
class MedicineOrderController extends GetxController {
  ...
  int isMedicine = Get.arguments['isMedicine'];

  String isChargeName = Get.arguments['isChargeName'];
  String isChargeValue = Get.arguments['isChargeValue'];
  ...
  orderMedicine() async {
    ...
    String labCharge ="${StorageService.readData(
        key: isChargeName == "Home Visit".tr? LocalStorageKeys.homeVisitCharge : LocalStorageKeys.labVisitCharge) ??
        "0"}" ?? "0";
  ...
  orderReport() async {
    ...
    for (int i = 0; i < cartList.length; i++) {
      ...
      double itemTotal = quantity * price;
      subtotal += itemTotal;
      tax_value =double.parse("${((subtotal+int.parse(labCharge))*int.parse(tax_pr.toString()))/100}").toStringAsFixed(2);

      testJson +=
      "\"${cartList[i].mId}\":{\"laboratory_id\":${cartList[i].pId
      },\"id\":${cartList[i].mId
      },\"title\":\"${cartList[i].name
      }\",\"quantity\":\"${qList[i]
      }\",\"price\":${int.tryParse(cartList[i].price ?? '0') ?? double.tryParse(cartList[i].price ?? '0.0'
    ...
    final url = Uri.parse("${Apis.ServerAddress}/api/report_order");

    final body = {
      "user_id": "${StorageService.readData(key: LocalStorageKeys.userId)}",
      "laboratory_id": "${cartList.first.pId}",
      "phoneno": phoneController.text,
      "total": (price.value % 2 == 0
          ?  price.value.toStringAsFixed(1)
          : price.value.toStringAsFixed(1)),
      ...
      "tax": tax_value.toString(),
      "tax_pr": tax_pr,
      "delivery_charge": labCharge,
      "address": isChargeName == "Home Visit".tr ? "${a!.address}" : isChargeName,
      "message": nameController.text,
    };

    // printCurlRequest(url, body);
  ...
  orderReportOnLine({required String tid, required String type}) async {
    ...
    for (int i = 0; i < cartList.length; i++) {
      ...
      double itemTotal = quantity * price;
      subtotal += itemTotal;
      tax_value ="${((subtotal+int.parse(labCharge))*int.parse(tax_pr.toString()))/100}";
    ...
    var response = await post(
      ...
      Uri.parse("${Apis.ServerAddress}/api/report_order"),
        ...
        if (result["status"] == 'success') {
          ...
          customDialog01(
            ...
            onPressed: () {
              ...
              orderReportOnLine(tid: result['tid'],type: "paystack");
        ...
        if (result["status"] == 'success') {
          ...
          customDialog01(
            ...
            onPressed: () {
              ...
              orderReportOnLine(tid: result['tid'],type: "braintree");
              Get.offAll(UserHomeScreen());
              // Get.back();
              // Get.back();
              // Get.back();
        ...
        if (result["status"] == 'success') {
          ...
          customDialog01(
            ...
            onPressed: () {
              ...
              orderReportOnLine(tid: result['tid'],type: "razorpay");
        ...
        if (result["status"] == 'success') {
          ...
          customDialog01(
            ...
            onPressed: () {
              ...
              orderReportOnLine(tid: result['tid'],type: "rave");
              Get.offAll(UserHomeScreen());
            },
          );
  ...
  void printCurlRequest(Uri url, Map<String, dynamic> body) {
    final curl = StringBuffer();
    curl.write('curl -X POST ');
    curl.write('"$url" ');
    curl.write('-H "Content-Type: application/x-www-form-urlencoded" ');
    curl.write('--data ');
    final dataString = body.entries
        .map((e) => '${Uri.encodeComponent(e.key)}=${Uri.encodeComponent(e.value.toString())}')
        .join('&');
    curl.write('"$dataString"');
    print("cURL Command:\n$curl");
  }
