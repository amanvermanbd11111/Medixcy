...
class ChatController extends GetxController {
  ...
  Future<void> uploadFileToServer1(
      Uint8List result, String extension, String type, String path) async {
    chatCircular(s1: "update_media_dialog".tr, dismiss: false);
    ...
    await getExternalStorageDirectory().then((value) async {
      ...
      try {
        Response response = await dio.post(
          "${Apis.ServerAddress}/api/mediaupload",
          data: formData,
          onSendProgress: (sent, total) {
            int progress = ((sent / total) * 100).toInt();
            message.value = "Uploading... $progress%";
          },
        );
        // Call uploadDataWithBackgroundService after successful upload
        uploadDataWithBackgroundService(tag, response.data);
      } catch (e) {
        Get.back();
        errorDialog(message: "error_s".tr+"error2".tr);
  ...
  uploadDataWithBackgroundService(String taskId, result) async {
    ...
    try {
      ...
      await collectionReference
        ...
        if (docSnapshot.exists) {
          await collectionReference.doc(taskId.toString()).update({
            ...
            "time": DateTime.now().toString(),
            "uid": myUid.value,
            "type": result['data'].toString().contains(".jpg") ||
                    result['data'].toString().contains(".png") ||
                    result['data'].toString().contains(".jpeg")
                ? 1
                : 2,
          });
        } else {
          await collectionReference.doc(taskId.toString()).set({
            "msg": result['data'],
            "time": DateTime.now().toString(),
            "uid": myUid.value,
            "type": result['data'].toString().contains(".jpg") ||
      ...
      if (isFirstMessage.value) {
        ...
        await dbRef.set({
          "time": DateTime.now().toString(),
          "last_msg": result['data'],
          "type": result['data'].toString().contains(".jpg") ? 1 : 2,
          "messageCount": 0,
          "status": 1,
          "channelId": channelId.value
        });
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  s1,
                  style: const TextStyle(
                      fontFamily: AppFontStyleTextStrings.black, fontSize: 14),
                ),
              ),
            ],
          ),
