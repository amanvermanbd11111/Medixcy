...
class DMoreInfoController extends GetxController {
  ...
  fetchLaboratoryReportDetails() async {
    isErrorInLoading.value = false;
    isLoaded.value = false;
    final response = await get(Uri.parse(
        "${Apis.ServerAddress}/api/laboratory_report?laboratory_id=${doctorId.value}"))
        .timeout(const Duration(seconds: Apis.timeOut))
        .catchError((e) {
      isErrorInLoading.value = true;
    });
  ...
  void onInit() async {
    ...
    loginKey.value = StorageService.readData(
        ...
        key: LocalStorageKeys.isLoggedInAsLaboratory) ??
        false
        ? labKey
        : doctorKey;
    ...
    } else if (loginKey.value == doctorKey) {
      await fetchDoctorDetails();
    } else {
      await fetchLaboratoryDetails();
      await fetchLaboratoryReportDetails();
    }
    if (isEdit.value == true) {
      if (loginKey.value == pharmacyKey) {
        getMedicineData(int.parse(medicineId.value));
      } else {
        getReportData(int.parse(medicineId.value));
      }
    }
  }
          ...
          child: Column(
            children: [
              ...
              CustomButton(
                onTap: () {
                  isEdit.value == false
                      ? uploadMedicineData(context)
                      : editMedicineData();
                },
                btnText: isEdit.value
        ...
        child: Column(
          children: [
            ...
            CustomButton(
              onTap: () {
                isEdit.value == false
                    ? uploadReportData(context)
                    : editReportData();
              },
              btnText:
              isEdit.value ? 'edit_report_str'.tr : 'add_report_str'.tr,
              margin: const EdgeInsets.fromLTRB(20, 0, 20, 5),
              textStyle: Theme.of(context).textTheme.bodyLarge!.apply(
                  color: Theme.of(context).scaffoldBackgroundColor,
                  fontSizeDelta: 2),
  ...
  uploadMedicineData(BuildContext context) async {
    ...
    if (feeController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please enter fee')),
      );
      return;
    }
    ...
    if (sImage == null) {
      final response = await post(
          Uri.parse("${Apis.ServerAddress}/api/add_pharmacy_medicine"),
          body: {
            "medicine_id": 0.toString(),
            "pharmacy_id": doctorId.value.toString(),
            "name": nameController.text.toString(),
            "price": feeController.text.toString(),
            "description": aboutUsController.text.toString(),
      ...
      var response = await d
          .post("${Apis.ServerAddress}/api/add_pharmacy_medicine",
          data: formData)
          .timeout(const Duration(seconds: Apis.timeOut))
          .catchError((e) {
        Get.back();
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['status'].toString() == "1") {
          ...
          Get.back();
          messageDialog('error'.tr, response.data['msg'].toString());
        }
      }
    }
  ...
  uploadReportData(BuildContext context) async {
    if (nameController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Please enter name')),
      );
    ...
    final response = await post(
        Uri.parse("${Apis.ServerAddress}/api/add_laboratory_report"),
        body: {
          "report_id": 0.toString(),
          "laboratory_id": doctorId.value.toString(),
          "name": nameController.text.toString(),
          "price": feeController.text.toString(),
          "description": aboutUsController.text.toString(),
        }).timeout(const Duration(seconds: Apis.timeOut)).catchError(
  ...
  editReportData() async {
    ...
    if (sImage == null) {
      final response = await post(
          Uri.parse(
              "${Apis.ServerAddress}/api/add_laboratory_report?medicine_id=${medicineId.value}"),
          body: {
            "report_id": medicineId.value,
  ...
  getMedicineData(medicine_id) async {
    ...
    final response = await post(Uri.parse(
        "${Apis.ServerAddress}/api/medicines_detail?medicine_id=$medicine_id"));
  ...
  getReportData(medicine_id) async {
    isEdit.value = true;
    medicineId.value = medicine_id;
    await Future.delayed(Duration.zero);
    final response = await post(Uri.parse(
        "${Apis.ServerAddress}/api/report_detail?report_id=$medicine_id"));
  ...
  deleteReport(medicine_id){
    // http://192.168.1.32/krupa/doctorlab/api/delete_pharmacy_medicine?medicine_id=115
  }
